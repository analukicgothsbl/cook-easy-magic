  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formattedRecipes: RecipeWithMeta[] = (data || []).map((recipe) => ({
          id: recipe.id,
          title: recipe.title,
  ...
  if (recipes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center p-6">
        ...
        <h3 className="text-xl font-semibold text-foreground mb-2">Library is empty</h3>
        <p className="text-muted-foreground">No recipes in the library yet!</p>
      </div>
    );
  }
